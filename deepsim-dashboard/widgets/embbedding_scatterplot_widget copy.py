...
class ScatterplotWidget(QWidget):
    ...
    def __init__(self, points, config):
        ...
        self.plot_widget.plot(self.points[:, 0], self.points[:, 1], pen=None, symbolBrush=self.points_color,
                              symbolSize=self.points_size)
    ...
    def find_nearest_neighbors(self, point, n=3):
        indices = self.get_indices_from_point(point)
    ...
    def draw_scatterplot(self):
        ...
        if len(self.selected_points) == 1:
            point = self.points[self.selected_points[0]]
            self.plot_widget.plot([point[0]], [point[1]], pen=None, symbolBrush=self.selection_color,
                                  symbolSize=self.selection_points_size)
